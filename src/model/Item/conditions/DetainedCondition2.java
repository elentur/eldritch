...
public class DetainedCondition2 extends DetainedCondition {
    ...
    protected Encounter getSpecialEncounter() {
        Test test = new Test(TestType.STRENGTH, -1, 1, SituationType.TEST);
          test.getEffect()[0][1]= new And(new Discard(this), new NextInvestigator());
        test.getEffect()[0][2] = new And(
                new LooseOrGainHealthSanity(SpendType.HEALTH, -3, GameService.getInstance().getEncounteringInvestigator()),
                new GainCondition(ConditionType.INJURY,getOwner()),
                new Discard(this),
